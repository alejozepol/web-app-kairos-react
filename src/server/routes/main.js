...
const API = 'https://www.api-kairos.extrasistemas.com/api';
...
async function init(req, res, next) {
    try {
        ...
        const html = renderToString( < Provider store = { store } >
            ...
            <
            Layout location = { req.url } > { ' ' } { renderRoutes(Routes) } { ' ' }

            <
            /Layout> <
            /StaticRouter> <
            /Provider>);
            const preloadedState = store.getState(); res.send(render(html, preloadedState));
        }
        catch (error) {
            next(error);
        }
